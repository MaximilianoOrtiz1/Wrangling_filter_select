...
After running the code, I found out that there are 8 batman movies. 
...
I believe the second code is wrong because it is in the wrong order.
...
After running the codes, I found out that the longest movie is The Cure For Insomnia (5220 minutes). While the oldest movie was Black Smith Scene (1893).
...
After running the above code, I figured out that there are 1248 movies made in 1995.

2. Which Comedies with an MPAA rating, over 120 minutes have a rating above 8?

```{r}
movies %>%
  filter(Comedy == 1, rating > 8, length > 120,mpaa>0) %>%
  select(title, rating, length,mpaa)
```

After running the above code I found out that there is 4 movies under those categories.
...
After running the code I found out there are 3 movies of Harry Potter includeed (Harry Potter and the Chamber of Secrets, Harry Potter and the Prisoner of Azkaban, Harry Potter and the Sorcerer's Stone). The highest rating movie was Harry Potter and the Prisoner of Azkaban  7.8, while the movie with the highest votes was The Sorcerer's Stone with 46741 votes.
